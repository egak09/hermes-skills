...
metadata:
  hermes:
    tags: [crypto, news, market-data, on-chain, defi]
...
> **Hermes on Windows (GFW)**: Use Python `requests` with proxy `http://127.0.0.1:1081` instead of raw `curl` — the proxy is required for API access. Prefer `execute_code` tool for parallel requests.
